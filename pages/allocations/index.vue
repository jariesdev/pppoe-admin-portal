  ...
  <div>
    <TableCard title="Allocations">
      <p>
        ...
        <nuxt-link to="job-orders">Job Orders</nuxt-link>
        section. For complimentary vouchers, <nuxt-link to="/allocations/complimentary">click here</nuxt-link>.
      </p>
      <ServerTable ref="mainTable" :headers="tableHeaders" :query-params="queryParams" url="/api/allocations">
        <template #item_is_sold="{row,value}">
          <span v-if="value" class="badge badge-success" title="Already sold">Yes</span>
          <button v-else class="badge badge-light" title="Click to mark as sold" @click="markAsSold(row)">No</button>
        </template>
      ...
      <el-dialog :before-close="(done)=>{ showConsumptionFor=null; done()}" :visible="showConsumptionFor!==null"
                 title="Consumption Details">
        <VoucherConsumption v-if="showConsumptionFor" :allocation-id="showConsumptionFor"/>
        <div class="text-center">
          <base-button @click="showConsumptionFor=null">Close</base-button>
        </div>
      </el-dialog>
    </TableCard>
  </div>
