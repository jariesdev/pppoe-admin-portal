<template>
  <div>
    <TableCard title="Network Access Servers (NAS)">
      ...
      <ServerTable ref="mainTable" :headers="tableHeaders" :query-params="queryParams" url="/api/nas">
        <template #item_nasname="{row: {id},value}">
          <nuxt-link :to="`nas/${id}`">{{ value }}</nuxt-link>
        </template>
    ...
    <card>
      <base-button type="primary" @click="addNAS">Register new NAS</base-button>
      <el-dialog :visible.sync="visibleNasFormDialog" show-close>
        <template #title>
          New NAS
        </template>
        <NasForm v-if="visibleNasFormDialog" @cancel="nasFormCancel" @success="nasFormSuccess"/>
      </el-dialog>
    </card>
  </div>
</template>
